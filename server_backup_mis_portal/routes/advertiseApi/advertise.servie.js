...
dummyHitTo: async(postbackUrl,service,client, callback) => {
  ...
  const fetchDataQuery = `SELECT * FROM tbl_advertisertraffic WHERE DATE(DATE)=DATE(SUBDATE(NOW(),0)) AND clientName='${client}' AND serviceName='${service}' ORDER BY DATE DESC`;
